...
// 3-step publish wizard wrapper.
// Manages step state and passes data between steps.
// Step 1 — Story details (title, hook, description, rating, status)
// Step 2 — Cover image upload
// Step 3 — Chapter selection + confirm
...
export default function PublishWizard({ project, chapters }: PublishWizardProps) {
  ...
  const [success, setSuccess] = useState<string | null>(null) // published story slug
            ...
            {STEPS.map((label, i) => {
              ...
              return (
                <div key={n} className="flex items-center gap-2">
                  ...
                  {i < STEPS.length - 1 && (
                    <div className="w-6 h-px bg-stone-200 mx-1" />
                  )}
                </div>
              )
            })}
        ...
        <div className="max-w-lg mx-auto w-full px-6 py-10 flex-1">
          ...
          {step === 2 && (
            <StepCover
              ...
              onCoverChange={(url: string | null) => updateData({ cover_url: url })}
              onNext={() => setStep(3)}
              onBack={() => setStep(1)}
            />
          )}
          ...
          {step === 3 && (
            <StepChapters
              ...
              onChange={(ids: string[]) => updateData({ chapter_ids: ids })}
