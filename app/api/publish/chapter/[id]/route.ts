...
export async function POST(
  _req: NextRequest,
  { params }: { params: Promise<{ id: string }> } // Next.js 15 Async Params Type
) {
  try {
    ...
    console.error('[PUBLISH_CHAPTER_POST_ERROR]:', error)
...
export async function DELETE(
  ...
  try {
    ...
    console.error('[PUBLISH_CHAPTER_DELETE_ERROR]:', error)
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 })
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    console.error('[PUBLISH_CHAPTER_PATCH_ERROR]:', error)
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 })
  }
}